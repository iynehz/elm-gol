...
initialize size = initializeBlank size


initializeBlank : Size -> Grid
...
clear : Grid -> Grid
clear grid = initializeBlank (Size (Array2D.rows grid) (Array2D.columns grid))


toIndexedList : Grid -> List (Position, Bool)
toIndexedList grid =
    let
        cellsArray = Array2D.indexedMap (\x y cell -> (Position x y, cell)) grid
    in
        Array.toList cellsArray.data
        |> List.foldl (\a b -> b ++ (Array.toList a)) []

set : Position -> Bool -> Grid -> Grid
set p = Array2D.set p.x p.y
